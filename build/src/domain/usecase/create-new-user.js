...
async function createNewUser(usersDetails) {
    const userExist = await (0, TB_UsersRepository_1.findUserByName)(usersDetails.username);
    if (userExist)
        throw new Error('USER_ALREADY_EXIST');
    const userResult = await (0, TB_UsersRepository_1.createUser)(usersDetails.username, usersDetails.email, usersDetails.password);
    const { Id } = JSON.parse(JSON.stringify(userResult));
    const net2eResult = await (0, Net2eRepository_1.addNet2e)(Id, usersDetails.username, usersDetails.password, usersDetails.secretQuestion, usersDetails.secretAnswer);
