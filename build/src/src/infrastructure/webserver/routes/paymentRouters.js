...
router.get('/createPaymentPaypal', async (req, res) => {
    try {
        await payment_1.makeRequest(res, payment_1.makeSubscription(parseInt(req.query.amount.toString()), req.query.silkQuantity.toString(), req.query.username.toString()));
    }
    catch (e) {
        console.log(e);
        res.status(500);
...
router.get('/executePaymentPaypal', async (req, res) => {
    try {
        const paymentSuccess = await payment_1.executePayment(res, req.query.token.toString(), req.query.silkQuantity.toString());
        if (paymentSuccess) {
